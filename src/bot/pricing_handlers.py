...
DUMP_MIN_CHARS = 4000
# Заглушка заменяет ВЕСЬ результат инструмента, а с ним и приложенные картинки прайса —
# они висят в том же блоке. Про них сказано прямо: иначе модель, увидев в тексте маркер
# «⟨ИЗОБРАЖЕНИЕ #22⟩», будет рассуждать о картинке, которой перед ней уже нет.
DUMP_STUB = ("[Выгрузка листа прайса убрана из истории вместе с приложенными картинками, "
             "чтобы не раздувать контекст. Ничего не потеряно: нужный кусок и логотипы "
             "перечитай через read_price_file (параметры sheet / contains / from_row).]")

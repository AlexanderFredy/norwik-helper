...
# ЗДЕСЬ ТОЛЬКО ПРИЧИНА И ДЕЙСТВИЕ ВЛАДЕЛЬЦА — как продолжить работу, говорит подсказка
# внизу сообщения (`_resume_hint` в pricing_handlers). Она читает состояние: есть ли план,
# на какой коллекции остановились, лежит ли файл на сервере. Дублировать это здесь нельзя:
# текст ошибки статичен и неминуемо разойдётся с реальностью — так и вышло с советом
# «пришлите файл заново», который пережил появление сохранения прайсов на диск и стал
# противоречить подсказке в том же сообщении.

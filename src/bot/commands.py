...
ADMIN_COMMANDS = COMMON_COMMANDS + [
    ...
    BotCommand(command="tokens", description="Расход на обращения к модели"),
...
<b>Расход на модель</b>
/tokens — сколько стоят обращения к модели: за сегодня, за неделю и по текущему
прогону, отдельно прайсы и запросы менеджеров. Там же доля префикса, отданного из
кеша: если она близка к нулю, значит кеширование сломалось и прогон переплачивает.

<b>Режим работы</b>
Прайс даёт два повода к правкам — расхождения в справочнике и изменения цен.
/mode — показать текущий режим
/mode товары+цены — сначала расхождения справочника, потом цены (по умолчанию)
/mode товары — только справочник, предложений по ценам не будет
/mode цены — только цены; замечания, не связанные с ценами, я не показываю

...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "АРТИКУЛОВ В ПРАЙСЕ НЕТ ВОВСЕ — передай `titles`, названия расцветок. Код "
            "сверит по ним ВНУТРИ этой коллекции. Ключ слабее артикула: другое написание "
            "декора в следующем прайсе не совпадёт, поэтому коды у поставщика всё равно "
            "стоит запросить.\n"
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "titles": {
                    "type": "array",
                    "description": ("названия расцветок этой коллекции — нужны, ТОЛЬКО "
                                    "если артикулов в прайсе нет вовсе"),
                    "items": {"type": "string"},
                },
                "prices": {
                    "type": "object",
...
class TaskBuilderTools:
    ...
    def _by_title(self, collection: str, live: list, titles, tm: str = "") -> dict | None:
        ...
        ни у одной позиции нет артикула, и сверка по артикулу отвечает «не нашлось
        ничего» — то есть не отвечает.

        ГРАНИЦЫ, за которые этот ключ не выходит (решение админа 24.09.2026):

        * только ВНУТРИ ОДНОЙ коллекции, уже опознанной по имени папки. Через марку
          целиком — никогда: «Дуб Медовый» есть и в Vivace, и в Modern, и связать строку
          прайса с карточкой чужой коллекции значит увезти туда цену;
        * неоднозначность = НЕ СОВПАЛО. Имя подошло к двум карточкам — говорим об этом, а
          не выбираем одну;
        * в сторону СНЯТИЯ не применяется: там наличие по-прежнему проверяется поиском по
          всему тексту файла;
    ...
    def _compare(self, inp: dict) -> str:
        ...
        **В ЭТОМ ВСЯ ЭКОНОМИЯ.** Номенклатура марки — тысячи токенов, и, попав в ответ
        ...
        # (3,6 мм) — ОДИН И ТОТ ЖЕ декор в двух толщинах, и артикулы у них общие:
        # «6006-4», «6009-20», «1002-32». Три кода прайса Spark нашлись в карточках
        # Modern, и дальше рушилось всё сразу: раз «нашлось», ветка поиска коллекции по
        # имени не запускалась, агент решал «коллекции SPARK в 1С нет» и предлагал
        # завести восемь позиций заново — при живых восьми; а сверка цен сравнивала
        # закупку Spark (1150) с ценой Modern (960) и показывала расхождение, которого
        # нет. Запись этих цен испортила бы чужую коллекцию.
        #
        # Разделяет случаи ровно одно: СУЩЕСТВУЕТ ЛИ в 1С коллекция с этим именем.
        # У Most Flooring её нет («Миллениум Про» против «Millenium Pro»), и там артикул
        # остаётся единственным ключом — эта ветка не срабатывает. У Spark она есть, и
        # тогда совпадение за её пределами — это ОМОНИМ, а не та же позиция.
        ...
        out = {
            ...
            # ОМОНИМЫ НАЗЫВАЕМ ПОИМЁННО: молча отброшенное совпадение агент прочтёт как
            # «артикула нет в 1С» и предложит завести позицию, которая там есть.
...
КАК ОФОРМЛЯТЬ ОПИСАНИЕ. Его читает человек — в форме 1С, в поле высотой в пять строк, и
глазами. Сплошной абзац на десять строк там не читается вовсе: в нём теряется и главное, и
предупреждение в конце.

— Пиши АБЗАЦАМИ по смыслу, между абзацами ПУСТАЯ СТРОКА.
— Любое перечисление — СПИСКОМ: каждый пункт с новой строки, начиная с «— ». Артикулы,
  свойства из шапки, замечания. Через запятую в строку — нельзя.
— Первой строкой — суть в одном предложении: что сделать и сколько позиций.
— Спорное и предупреждения — ПОСЛЕДНИМ абзацем, чтобы их было видно.

Порядок абзацев: суть → откуда это (раздел, лист) → что показала сверка числом → список
позиций → свойства из шапки → цены → на что обратить внимание.

Образец:

    Завести коллекцию «Anatolia Elite» целиком — 12 позиций.

    Раздел «ANATOLIA ELITE 33 КЛАСС 12 ММ», лист «ламинат Турция».
    Сверка: ни один из 12 артикулов в 1С не нашёлся.
    ...
    Свойства из шапки:
    — размер 1290*157*12 мм
    — класс 33, V-фаска, тиснение в регистр EIR
    — упаковка 6 шт / 1,215 м2

    Цены: закупка 1890, МРЦ 2990 руб/м2.

    Внимание: артикулы AN 901–AN 911 повторяются в соседнем разделе ANATOLIA DESIGN
    (8 мм) — различать по толщине.
...
— «добавление новых» — сверка нашла артикулы прайса, которых нет в 1С. Перечисли их.
  АРТИКУЛОВ В ПРАЙСЕ НЕТ ВОВСЕ (бывает: коллекция «Westerhof Vivace» на листе SPC) —
  передай в сверку `titles`, названия расцветок, и смотри на `сверка_по_названиям`.
  Заводить надо только то, что в `нет_в_1С_по_названию`. И скажи в описании, что кодов у
  поставщика нет: сверка по именам держится на написании декора и переживёт не всякий
  следующий прайс.
  **ПРИШЛО `артикулы_заняты_другой_коллекцией` — ЭТО ОМОНИМЫ, а не находки.** Один и тот
  же код декора бывает у двух коллекций одной марки в разных толщинах: «6006-4» есть и у
  Spark (4 мм), и у Modern (3,6 мм). Такие совпадения код из сверки убрал сам. Не заводи
  по ним задачу на цены — цена чужой коллекции не менялась, — и не считай, что позиция в
  1С уже есть: в СВОЕЙ коллекции её нет.
...
def _flat(value: str) -> str:
    """Имя без регистра, пробелов и знаков — для сравнения названий расцветок.

    «Дуб  Медовый», «дуб медовый» и «Дуб-Медовый» — одно и то же название; «Авила» и
    «Дуб Авила» — РАЗНЫЕ, и склеивать их нельзя: это разные декоры у разных поставщиков.
    """

...
class TaskKind(str, Enum):
    ...
    **«Перенос в снятые» идёт ПЕРВЫМ** (решение админа 24.09.2026). Две причины. Первая:
    перед созданием позиции обязательна проверка, нет ли её среди снятых, иначе заводится
    дубль (§19.11 content-manager.md), — значит снятое должно уехать раньше заведения
    нового. Вторая: пока снятое лежит в живых папках, нормализация и свойства причёсывают
    позиции, которые вот-вот покинут марку, — работа впустую и лишние записи в 1С.

    Дальше — порядок убывающей общности: имена по марке целиком, свойства по коллекциям,
    заведение недостающих, цены. Цены последние намеренно: писать их позиции, которую ещё
    предстоит завести, некуда.
    """
...
class TaskSubject(str, Enum):
    """На что направлена задача.

    `MARK` — марка ЦЕЛИКОМ, без разбиения на коллекции. Нужен там, где дробить работу не
    по чему: нормализация наименований идёт по единому шаблону для всей марки, и пять
    одинаковых задач на пять коллекций — это пять захватов, пять прогонов и пять строк в
    списке вместо одной.
    """
    MARK = "марка"

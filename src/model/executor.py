...
TOOLS = [
    ...
    },
    {
        "name": "get_1c_items",
        "description": (
            "Номенклатура 1С по марке: [{ref, article, name, full_name, collection, "
            "collection_ref, product_type, unit, prices, properties, discontinued}]. "
            "Параметр tm_code обязателен, collection — необязательный фильтр по имени "
            "папки. ВЫЗЫВАЙ ОДИН РАЗ на марку: выгрузка большая и целиком едет в каждый "
            "следующий запрос."),
        "input_schema": {
            "type": "object",
...
class TaskTools:
    ...
    async def _items(self, inp: dict) -> str:
        ...
        if wanted:
            items = [i for i in items if wanted in (i.collection or "").lower()]

        if not items:
            return ("В 1С по этой марке ничего не найдено"

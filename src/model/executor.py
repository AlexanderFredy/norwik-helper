...
TOOLS = [
    ...
    },
    {
        "name": "write_prices",
        "description": (
            "ЗАПИСАТЬ цены по ОДНОЙ коллекции. Пишет в 1С СРАЗУ. Передавай цены ИЗ "
            "ПРАЙСА, приведённые к базовой ЕИ товара: purchase — закупочную, rrc — "
            "рекомендованную розничную (в прайсах её пишут как РРЦ или МРЦ), если она там "
            "есть. Розницу магазина НЕ передавай: её считают правила.\n"
...
class TaskTools:
    ...
    async def _write_prices(self, inp: dict) -> str:
        ...
        if rows:
            ...
            # РРЦ ПИШЕТСЯ, ЕСЛИ ОНА В ПРАЙСЕ ЕСТЬ. Раньше сюда жёстко шёл None, и агент
            # честно докладывал: «РРЦ отдельным полем не записывалась — инструмент цен
            # принимает только закупочную». Планировщик её умел всегда, не хватало входа.
            # Без неё круг не закрывался: сверка при пересборке снова показывала бы
            # расхождение по РРЦ, а починить его было нечем.
            rrc = inp.get("rrc")
            group = plan_price_group(in_collection, tm_code, tm_name,
                                     Decimal(str(purchase)),
                                     None if rrc is None else Decimal(str(rrc)),
                                     date.today())
            missing = []

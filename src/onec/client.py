...
#:
#: Поэтому здесь СТАРТОВЫЙ размер, а не приговор: клиент сам ужимает его вдвое, если по
#: каналу крупная страница не доходит (`_page_failed`), и работает так до перезапуска.
#: Константа хороша ровно для одного канала, а канал меняется — через VPN не проходило и
#: восемь позиций, напрямую сервер отдаёт двести за 6,8 с. Подбор снимает выбор между
#: «быстро» и «доедет»: быстро там, где доезжает.
#:
#: Шестьдесят четыре — из того же журнала: раз двести позиций считаются 6,8 с, то 64
#: укладываются в 3–4 секунды при `PAGE_TIMEOUT` в 12. Марка из 240 позиций забирается
#: четырьмя запросами вместо тридцати.
#:
#: Степень двойки ОБЯЗАТЕЛЬНА: половины страницы адресуются той же арифметикой страниц
#: (`_rescue`), и на нечётном размере границы разъехались бы, потеряв позицию молча.
PAGE_SIZE = 64
...
class OnecClient:
    ...
    def __init__(self, base_url: str, token: str, timeout: float = 30.0,
        ...
        # Размер страницы выгрузки ПОДБИРАЕТСЯ ПОД ПУТЬ и живёт на клиенте, а не на
        # вызове: начинаем с `PAGE_SIZE`, а если крупная страница по этому каналу не
        # доходит — ужимаемся вдвое и работаем так до перезапуска (`_page_failed`).
        # Константа хороша ровно для одного канала, а канал меняется: через VPN не
        # проходило и восемь позиций, напрямую сервер отдаёт двести за 6,8 с.
    ...
    def by_tm_all(self, tm_code: str, size: int | None = None, max_pages: int = 500,
        ...
        клиент помнит, какой размер проходит по этому каналу (`_page_failed`). Без него
        берётся подобранный.

        `max_pages` — потолок ЗАПРОСОВ, а не страниц, и он велик намеренно: когда размер
        ужимается до единицы, марка из двухсот позиций требует двухсот запросов, а
        оборванная на полпути выгрузка опаснее долгой. Недостающую позицию агент прочтёт
        как «товара в 1С нет» и заведёт дубль.
        ...
        **НЕ ОТДАЛАСЬ — ДЕЛИМ ПОПОЛАМ**, а не разбираем по одной. Разбор по одной стоил
        ...
        tm_name = ""
        total: int | None = None
        start = 1                                       # номер следующей позиции, с единицы

        for _ in range(max_pages):
            # Размер берётся на КАЖДОМ шаге: он мог уменьшиться на предыдущей странице.
            step = self._page_size
            page = (start - 1) // step + 1
            try:
                chunk = self._ask_page(tm_code, page, step, kw)
                ...
                logger.warning("Страница %d марки %s не отдалась — делим пополам",
                               page, tm_code)
                seen = self._rescue(tm_code, page, step, kw, items, errors)
                if seen is not None:
                    total = seen
                self._page_failed(step)
            else:
            ...
            # ШАГ ВСЕГДА ПО СТАРОМУ РАЗМЕРУ: страница была запрошена им, и именно её
            # границы мы прошли. Уменьшение размера вступает в силу со следующего шага, и
            # границы не разъезжаются ровно потому, что размер только ДЕЛИТСЯ ПОПОЛАМ:
            # пройденный рубеж кратен старому размеру, а значит и новому.
            start += step
    ...
    def _page_failed(self, size: int) -> None:
        """Страница такого размера не дошла. Дважды подряд — размер уменьшается вдвое.

        **ДВАЖДЫ, а не однажды.** Одиночный провал на плохом пути случаен (соседние
        запросы проходят за секунду), и ужиматься из-за него значит остаться на мелкой
        странице до конца прогона на ровном месте.

        Размер только УМЕНЬШАЕТСЯ и живёт до перезапуска процесса. Отращивать его обратно
        пробами нельзя: каждая неудачная проба стоит полного ожидания, а путь меняется
        реже, чем идут выгрузки. Сменили канал — перезапустили бота, и он снова начнёт с
        `PAGE_SIZE`.
        """
        if size < self._page_size or self._page_size <= 1:
            return                                      # провалилась половина, не страница
        self._page_fails += 1
        if self._page_fails < 2:
            return
        self._page_fails = 0
        self._page_size = max(1, self._page_size // 2)
        logger.warning("Страница уменьшена до %d позиций: по этому пути крупная не "
                       "доходит. Вернуть прежний размер — перезапуск бота",
                       self._page_size)

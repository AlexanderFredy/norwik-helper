...
class OnecClient:
    ...
    def by_tm_all(self, tm_code: str, size: int = PAGE_SIZE, max_pages: int = 80,
        ...
        **НЕ ОТДАЛАСЬ — ДЕЛИМ ПОПОЛАМ**, а не разбираем по одной. Разбор по одной стоил
        ...
        """
        # Предел ожидания СЧИТАЕТСЯ ПО РАЗМЕРУ запроса (`_page_once_more`), а не хранится
    ...
    def _page_once_more(self, tm_code: str, page: int, size: int, kw: dict):
        ...
        try:
            ...
            logger.info("Страница %d марки %s (размер %d) не ответила — пробуем ещё раз",
                        page, tm_code, size)
            return self.by_tm(tm_code, page=page, size=size, timeout=wait, **kw)
    ...
    def _rescue(self, tm_code: str, page: int, size: int, kw: dict,
        ...
        if size <= 1:
            ...
            try:
                one = self._page_once_more(tm_code, index, 1, kw)
            except httpx.TimeoutException:
                errors.append({
        ...
        for part in (page * 2 - 1, page * 2):
            ...
            if total is not None and (part - 1) * half >= total:
                break
            try:
                chunk = self._page_once_more(tm_code, part, half, kw)
            except httpx.TimeoutException:

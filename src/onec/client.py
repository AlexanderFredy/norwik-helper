...
#: Позиций в одной странице выгрузки номенклатуры. Подбиралось по живой базе: на 25
#: позициях ответ приходит за 2–3 секунды, на 200 — упирается в таймаут и держит 1С.
PAGE_SIZE = 25
...
class OnecClient:
    ...
    def by_tm_all(self, tm_code: str, size: int = PAGE_SIZE, max_pages: int = 80,
        ...
        **СТРАНИЦА МАЛЕНЬКАЯ НАМЕРЕННО.** Обработчик собирает цены ПО КАЖДОЙ позиции,
        тремя видами цен, — это отдельные запросы к периодическому регистру, и стоимость
        страницы растёт вместе с ней. Прося 200 позиций разом, мы занимали базу на минуту
        и упирались в собственный таймаут, а заодно блокировали все прочие запросы к 1С:

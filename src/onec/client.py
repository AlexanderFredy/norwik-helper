...
#:
#: ВЫШЕ ПОДНИМАТЬ ПОКА НЕЛЬЗЯ, и причина не в нагрузке. На здоровом сервисе (ноль
#: провалов из шестидесяти подряд) лестница размеров идёт 1,3 → 1,4 → 1,6 с на 1, 8 и 16
#: позициях, но у Вестерхофа со снятыми есть сочетание, которое НЕ ОТВЕЧАЕТ НИКОГДА —
#: двенадцать провалов из двенадцати:
#:
#:     by-tm?tm=000000005&page=1&size=8&include_not_exported=1   молчит всегда
#:     by-tm?tm=000000005&page=1&size=7&include_not_exported=1   1,5 с, 11 619 байт
#:     by-tm?tm=000000005&page=1&size=8                          1,7 с, 12 823 байта
#:     by-tm?tm=000000005&page=2&size=4&include_not_exported=1   1,4 с (те же позиции 5-8)
#:
#: Объём ответа ни при чём (у другой марки доезжает 21 491 байт), кеш по URL ни при чём
#: (лишний параметр и порядок не меняют ничего), карточки здоровы (каждая из восьми
#: читается по одной).
#:
#: **И ОБРАБОТЧИК 1С ТУТ ТОЖЕ НИ ПРИ ЧЁМ.** На самом сервере по `127.0.0.1` тот же запрос
#: (и даже `size=150`) отдаётся за 1,5 с без единого сбоя — проверено админом. А снаружи
#: на провалившемся запросе не приходят ДАЖЕ ЗАГОЛОВКИ: ноль байт, ни строки статуса,
#: тогда как соседний удачный запрос отдаёт 21 491 байт за 1,8 с. Значит ломается путь
#: между рабочей станцией агента и IIS, а не запрос и не данные. Размер страницы держим
#: маленьким, пока путь не починен: крупный ответ по нему не проходит.
#:

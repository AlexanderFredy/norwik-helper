...
def keys_present(names: set, refs: set) -> bool:
    """Опознана ли коллекция в группе. Пусто — значит опознать нечем."""
    return bool(names or refs)
...
class PricingTools:
    ...
    async def _propose(self, inp: dict) -> str:
        ...
        if modes.with_items(self.mode):
            refusal = await self._items_gate(inp, run, stage, tm_code)
            if refusal:
                return refusal
    ...
    async def _items_gate(self, inp: dict, run: dict | None, stage: dict | None,
                          tm_code: str | None) -> str | None:
        """Пускать ли к ценам. Строка — отказ с объяснением, None — можно.

        Три проверки, все про порядок §19.7:
          * одна коллекция за вызов — иначе марка снова уедет одним куском;
          * коллекция названа — иначе проверять нечего и гейт бессилен;
          * по ней уже вызывали `propose_items`;
          * и по марке открыта очередь коллекций — иначе первая же коллекция закроет всю
            марку и прогон объявит прайс обработанным.
        """
        ...
        if not keys_present(names, refs):
            # БЕЗ ИМЕНИ КОЛЛЕКЦИИ ГЕЙТ БЕССИЛЕН, и это была дыра: группа, в которой не
            # передан ни `collection_ref`, ни `collection`, давала пустой набор ключей — и
            # проверка «сверял ли справочник» превращалась в «проверять нечего», то есть в
            # разрешение. Найдено на прогоне Most Floor 10.09.2026.
            #
            # Отказ безопасен: `propose_prices` и так отбирает позиции по `collection_ref`,
            # и группа без него не нашла бы в 1С ни одного товара.
            return ("В группе не указана коллекция. Передавай `collection_ref` — код папки "
                    "из выгрузки 1С (и `collection` для читаемости): по нему отбираются "
                    "позиции, и по нему же я проверяю, что справочник этой коллекции уже "
                    "сверен (§19.7).")

        # ОЧЕРЕДЬ КОЛЛЕКЦИЙ ОБЯЗАТЕЛЬНА. Без неё предложение по ОДНОЙ коллекции закрывает
        # ВСЮ марку (`mark_tm_done`), план пустеет, и прогон рапортует «прайс обработан
        # полностью». Ровно так 10.09.2026 у Most Flooring из восьми коллекций разобралась
        # одна: правило «одна коллекция за вызов» без очереди превращает марку в первую же
        # её коллекцию.
        #
        # Механизм не новый — тот же `stage`, которым дробятся крупные марки (§9.6.2).
        # В товарных режимах он нужен ВСЕГДА: по коллекциям тут идут независимо от объёма.
        planned = {t.get("code") for t in (run or {}).get("planned") or []}
        if tm_code and tm_code in planned and not (
                stage and stage.get("tm_code") == tm_code):
            # Формулировка та же, что у порога крупной марки ниже: отказ по сути один —
            # «целиком за один заход не берём», — и двух разных фраз про это быть не должно.
            return ("В режиме с правкой товаров разбираем по коллекциям, целиком за один "
                    "заход не берём. Вызови start_tm_collections, перечислив коллекции "
                    "этой марки, которые ЕСТЬ В ПРАЙСЕ, и дальше работай по одной: "
                    "propose_items → кнопка → propose_prices → следующая. Без очереди "
                    "первая же коллекция закроет всю марку, и прайс объявится обработанным.")

...
class BadCardTest(unittest.TestCase):
    ...
    def test_transient_hiccup_goes_straight_to_halves(self):
        """Сорвавшаяся страница НЕ ПОВТОРЯЕТСЯ целиком, а сразу делится.

        Половины — это и повторная попытка, и полезная работа сразу, тогда как повтор
        страницы стоил второго полного ожидания: в боевом логе сорвавшаяся страница
        обходилась в полторы минуты при цене здоровой в полторы секунды.
        """
        ...
        self.assertEqual(nom.errors, [])
        # Четыре запроса: провалившаяся страница, две её половины и вторая страница.
        # Повтора страницы среди них НЕТ — он стоил бы ещё одного полного ожидания.
